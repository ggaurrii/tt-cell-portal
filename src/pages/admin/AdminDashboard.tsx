      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          <Link to="/admin/reports">
            <button className="btn-primary flex items-center gap-2 text-sm py-2 px-4 rounded-xl shadow-sm transition-all">
        ...
        <Card className="flex flex-col h-[420px]">
          ...
          <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={DOMAIN_DISTRIBUTION} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={3} dataKey="value">
              ...
              lowAttendance.map(s => (
                <div key={s.id} className="flex items-center gap-3 pb-3 border-b border-surface-border/50 last:border-0 last:pb-0 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800 truncate transition-colors">{s.name}</p>
                    <p className="text-xs text-red-500 font-bold mt-0.5">{s.attendancePercentage}% attendance</p>
                  </div>
                  <Link to={`/admin/students`}>
                    <button className="text-[10px] font-bold text-military-dark bg-gray-100 px-3 py-1.5 rounded-md transition-colors uppercase tracking-wide">View</button>
                  </Link>
                </div>
              ))
            )}
        ...
        <Card className="flex flex-col h-[420px]">
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
            {MOCK_ALERTS_NOTICES.map((alert) => (
              <div key={alert.id} className="p-3.5 border border-surface-border rounded-xl transition-colors bg-white shadow-sm">
